<div class="dashmidMain">			
	...
	<h3>Event <?php if($ptype=='comment') {echo 'Comments'; } else {echo 'Posts';} ?> (Total Post : <?php echo $total_post;?>)</h3>
	<br />
	<div class="transFilt">
	<?php echo $this->Form->create('posts', array('action'=>'admin_post_list/'.$event_id)); ?>
		...
		<div class="appoiTable2">
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				  <tr>
					...
					<th width="20%"><?php echo $this->Paginator->sort("Comments",__("Comments")); ?></th>
				  ...
				  <tr class="<?php echo $tableClass;?>">
                    ...
                    //	echo $this->Html->image(SITE_PATH.'img/video-icon.png', array('alt' => 'post', 'border' => '0', 'width' => '30', 'height' => '30' , 'class'=>'picon'));
                     ...
                     ?></td>
                    <td><?php 
                    if($postData['Post']['type']=='comment') {
                    	echo $this->Html->link('Replies ('.$this->My->getCommentsReply($postData['Post']['id']).')',array('controller'=>'posts','action'=>'comment_reply_list',$postData['Post']['id']),array('rel'=> 'Reply List','data-placement' => 'left','data-original-title' => 'Edit','class'=> 'event_link','escape'=> false  ));
                    }
                     if($postData['Post']['type']=='video' || $postData['Post']['type']=='image') {
                    	echo $this->Html->link('Comments ('.$this->My->getCommentsOnMimePost($postData['Post']['id']).')',array('controller'=>'posts','action'=>'comment_list',$postData['Post']['id']),array('rel'=> 'Reply List','data-placement' => 'left','data-original-title' => 'Edit','class'=> 'event_link','escape'=> false  ));
                    }
                    ?></td>
                    <td><?php 
					...
					<?php $id = $postData['Post']['id']; 
					      $model = 'Post'; 
					?>
					<?php echo  $this->Html->image("admin/view-icon.png", array("alt" => "",'url' => array('controller' => 'posts', 'action' => 'view_post', 'admin' => true, $id,$postData['Post']['user_id'],$postData['Post']['event_id'])));?>
                     <?php
