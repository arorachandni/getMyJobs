<?php echo $this->Html->css('admin/datetimepicker/jquery.datetimepicker');
echo $this->Html->script('admin/datetimepicker/jquery.datetimepicker.full');
?>
<div class="dashmidMain">			
	...
	<br />
	<div class="transFilt">
	<?php echo $this->Form->create('reports',array('action'=>'admin_paymentList')); ?>
       	<div class="transFilt-L">
          
            <?php echo $this->Form->text('Payment.start_date', array('class' => 'testInp', 'label' => false, 'placeholder'=>'Start Date','required' => false, 'autocomplete'=>"off")); ?>
        </div>
		<div class="transFilt-L" >
            <?php echo $this->Form->text('Payment.end_date', array('class' => 'testInp', 'label' => false, 'placeholder'=>'End Date','required' => false, 'autocomplete'=>"off")); ?>
        </div> 
		
		<div class="transFilt-R" style='width:11%;'>
			<?php echo $this->Form->submit('Search', array('div'=>false, 'label'=>false, 'class'=>'submitBtn'));?>
		</div>
      
        <?php echo $this->Form->end(); ?>
		 <?php if (! empty($payment_list)){?>
		 <div class="transFilt-R"  style='width:auto;'>
				<?php echo $this->Html->link('Export CSV', 'javascript:void(0);',array('class' => 'submitBtn', 'id'=> 'exportcsv'));?>
		 </div>
		<?php } ?>	
		</div>
...
<script>
 $(document).ready(function(){
   ...
   $('#PaymentStartDate').datetimepicker({
	timepicker:false,
	format:'Y-m-d',
	formatDate:'Y-m-d'
	});
   ...
   $('#exportcsv').click(function(){
     var start = $('#PaymentStartDate').val();
	 var end = $('#PaymentEndDate').val();
	 window.location.href='<?php echo SITE_PATH;?>/admin/reports/exportPayment/'+start+'/'+end;
   });
 });
</script>