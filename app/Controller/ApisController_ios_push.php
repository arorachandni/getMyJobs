...
class ApisController extends AppController {
	...
	function beforeFilter() {
		...
		if (!empty($this->Auth))
			...
			'loginapp','updateProfile','getUserProfile','setFavourite','getUpdatedProfile','addPost','editPost','addPostComment','uploadmedia','getcountrylist','getEventDetail', 'verifyOTP','resendOTP', 'register', 
			'getEvents','addevent','editevent','getSearchEvents','getContactUsers','getCommingEventsMap','inviteFriends','getNotification','myEvents','postDetail','setEventFavourite','setPostFavourite','invities',
			'setAbusePost','getPage','updateDetail','cropImages','updateEmail','updateNumber','deleteEvent','deletePost','getMyFauouritePost','getAttendees',
			'addComment' ,'addReply','setAbuseEvent','postListing','updatePassword','updateNotification','invitationList','getNotificationSetting','checkUserFriend',
			'setIvitationStatus','Send_text_message','checkLatLng','forgetPassword','uploadImages','changeMobile','send_push_notification','iphonPN','setNotificationStatus');
	...
	*/
	...
	function loginapp(){
		if($this->request->is('post') && $this->data){
			...
			if(empty($this->data['username'])){
			...
			} else if(empty($this->data['device_token'])){
				$error='Please enter device token.';
			} else if(empty($this->data['device_type'])){
				$error='Please enter device type.';
			}  else {
				
				$password=$this->Auth->password($this->data['password']);
				$usersList = $this->User->find("all", array(
					'fields' => array('User.id', 'User.username','User.fullname','User.status','User.email','User.gender','User.deviceid','User.address','User.mobile','User.countrycode','User.iso','User.upload_id'),
					'conditions' => array(
				...
				if(!empty($usersList)){
					if($usersList[0]['User']['status']==2) {
						echo json_encode(array('status'=> '2', 'data'=> 'Please enter otp')); exit;
					} 
					$usersList[0]['User']['url']=$this->Api->fetchUserImage($usersList[0]['User']['id']);
					$usersList[0]['User']['cname']=$this->Api->fetchCountryName($usersList[0]['User']['iso']);
					$udata=array();
					$udata['User']['id']=$usersList[0]['User']['id'];
					$udata['User']['device_token']=$this->data['device_token'];
					$udata['User']['device_type']=$this->data['device_type'];
	...
	*/
	...
	function send_push_notification($registatoin_ids,$data) {
     ...
     function send_ios_push($deviceToken , $data ) {
		...
		$fp = stream_socket_client('ssl://gateway.sandbox.push.apple.com:2195', $err, $errstr, 60, STREAM_CLIENT_ASYNC_CONNECT|STREAM_CLIENT_CONNECT|STREAM_CLIENT_PERSISTENT, $ctx);
     	if (!$fp)
	...
	*/
	...
	function register(){
		...
		if($this->request->is('post') && $this->data){
			...
			if(empty($this->data['username'])){
			...
			} else if(empty($this->data['iso'])){
				$error='Please provide iso code.';  
			} else if(empty($this->data['password'])){
				$error='Please enter password.';  
			} else if(empty($this->data['device_token'])){
				$error='Please enter device token.';
				...
				$getMobiledata = $this->Api->checkExitsMobile($mobile,$ccode);
				if(!empty($getdata)){
					$response_arry = array('message'=> 'Username already registered', 'status'=> '0', 'error'=> '0');
				}else if(!empty($getMobiledata)){
					$response_arry = array('message'=> 'Mobile number already registered', 'status'=> '0', 'error'=> '0');
				}else{
				
					$data['User']['username'] = $this->data['username'];
					$ccode = $this->data['ccode'];
					$data['User']['iso'] = $this->data['iso'];
					$data['User']['countrycode'] = $this->data['ccode'];
					...
					$data['User']['device_token'] = $this->data['device_token'];
					$data['User']['device_type']=$this->data['device_type'];
					$data['User']['status'] = 2;
					
					$response = $this->send_sms('+'.$ccode.$data['User']['mobile'], $data['User']['otp']." is your OTP to signup to your Orber Account. Please do not share this with anyone.");
					...
					if($response == '1'){
						...
						$result=array();
						$result['Notification']['user_id'] = $user_id;
						$result['Notification']['comment'] = 3;
						$result['Notification']['favourite'] = 3;
						$result['Notification']['posts_posted_event_wall'] = 3;
						$result['Notification']['posts_posted_invited_event'] = 3;
						$result['Notification']['invitation'] = 2;
						$result['Notification']['date'] = date('Y-m-d H:i:s');
						$this->Notification->save($result);
						$response_arry = array('message'=> _MSG1003, 'status'=> '1', 'error'=> '0');
					}else{
						$response_array = array('message'=>'Please enter correct mobile number.', 'status'=> '0', 'error'=> '0');
					}
						...
						$profilepic=$this->Api->fetchUserImage($this->request->data['user_id']);
							$msg = array
							 (
								...
								'vibrate'	=> 1,
								'sound'		=> 1,
								'alert'     => $username. ' commented in your post at ' .$event_data['Event']['event_title'],
								'type'      =>'comment',
								'notify_id' => $notify_id,
								'icon'	=> 'http://flexsin.org/lab/mobilewebservice/orberCMS/img/userImg/'.$profilepic,
								'smallIcon'	=> 'http://flexsin.org/lab/mobilewebservice/orberCMS/img/userImg/'.$profilepic,
							...
							$result=$this->send_ios_push($device_token,$msg);
						...
						$profilepic=$this->Api->fetchUserImage($this->request->data['user_id']);
							$msg = array
							 (
								...
								'vibrate'	=> 1,
								'sound'		=> 1,
								'alert'		=> $username. ' posted in your ' .$event_data['Event']['event_title'],
								'notify_id' => $notify_id,
								'icon'	=> 'http://flexsin.org/lab/mobilewebservice/orberCMS/img/userImg/'.$profilepic,
								'smallIcon'	=> 'http://flexsin.org/lab/mobilewebservice/orberCMS/img/userImg/'.$profilepic,
								'launchImage' => 'http://flexsin.org/lab/mobilewebservice/orberCMS/img/userImg/'.$profilepic,
			...
			if(empty($this->data['user_id'])){
			    ...
			    $error='Please enter favourite status.';
					...
					$result=$this->send_push_notification($device_token,$fields);
							...
							$result=$this->send_ios_push($device_token,$fields);
					}
					//$eventInfo = $this->Api->findEventByID($eventId, $fields);
					...
					$result=$this->send_push_notification($device_token,$fields);
						...
						$msg = array
						 (
							...
							'type'      =>'invitation_notify',
							'vibrate'	=> 1,
							'alert' 	=> 	 $username.' '. $f .' your invitation of event '.$this->Api->fetchEventName($event_id),
							'sound'		=> 1,
							'notify_id' => $this->data['invitation_id'],
							'event_creater_id'=>$host_id,
							'event_id'=> $invitedata['Invite']['event_id'],
									...
									$result=$this->send_push_notification($device_token,$fields);
										...
										$msg = array
										 (
											...
											'type'      =>'eventinvitation',
											'vibrate'	=> 1,
											'alert'		=> $username. ' invite you to '. $this->Api->fetchEventName($event_id),
											'sound'		=> 1,
											'icon'	=> 'http://flexsin.org/lab/mobilewebservice/orberCMS/img/userImg/'.$profilepic,
											'smallIcon'	=> 'http://flexsin.org/lab/mobilewebservice/orberCMS/img/userImg/'.$profilepic,
											'launchImage' => 'http://flexsin.org/lab/mobilewebservice/orberCMS/img/userImg/'.$profilepic,
					...
					$profilepic=$this->Api->fetchUserImage($this->request->data['user_id']);
						...
						$msg = array
						 (
							...
							'type'      =>'postcomment',
							'vibrate'	=> 1,
							'alert'		=> $username. ' comment on your post at '.$event_name,
							'sound'		=> 1,
							'icon'	=> 'http://flexsin.org/lab/mobilewebservice/orberCMS/img/userImg/'.$profilepic,
							'smallIcon'	=> 'http://flexsin.org/lab/mobilewebservice/orberCMS/img/userImg/'.$profilepic,
							'launchImage' => 'http://flexsin.org/lab/mobilewebservice/orberCMS/img/userImg/'.$profilepic,
